...
package io.kindbrave.mnn.webserver.modelapi.download
...
import io.kindbrave.mnn.webserver.modelapi.download.ms.KindBraveMsModelDownloader
...
class KindBraveModelDownloadManager private constructor(private val context: Context) {
...
//    private val mlDownloader:MLModelDownloader
    private var msDownloader: KindBraveMsModelDownloader
    ...
    init {
        ...
        msDownloader = KindBraveMsModelDownloader(downloadCallback, cachePath)
    ...
    companion object {
        @Volatile
        private var instance: KindBraveModelDownloadManager? = null
        const val TAG: String = "ModelDownloadManager"

        const val REQUEST_CODE_POST_NOTIFICATIONS: Int = 998

        fun getInstance(context: Context): KindBraveModelDownloadManager {
            if (instance == null) {
                synchronized(KindBraveModelDownloadManager::class.java) {
                    if (instance == null) {
                        instance = KindBraveModelDownloadManager(context)
                    }
                }
            }
            return instance!!
