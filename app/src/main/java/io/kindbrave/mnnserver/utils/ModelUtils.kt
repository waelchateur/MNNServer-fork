...
object ModelUtils {
    ...
    fun generateBenchMarkString(metrics: HashMap<String?, Any?>): String {
        ...
        val promptLen = metrics.get("prompt_len") as Long
        val decodeLen = metrics.get("decode_len") as Long
        val prefillTimeUs = metrics.get("prefill_time") as Long
        val decodeTimeUs = metrics.get("decode_time") as Long
        // Calculate speeds in tokens per second
        val promptSpeed = if (prefillTimeUs > 0) (promptLen / (prefillTimeUs / 1000000.0)) else 0.0
    ...
    fun generateDiffusionBenchMarkString(metrics: HashMap<String?, Any?>): String {
        val totalDuration = metrics.get("total_timeus") as Long * 1.0 / 1000000.0
        return String.format("Generate time: %.2f s", totalDuration)
    }

    private val blackList: MutableSet<String?> = HashSet<String?>()
    ...
    private val hotList: MutableSet<String?> = HashSet<String?>()

    init {
        hotList.add("taobao-mnn/DeepSeek-R1-7B-Qwen-MNN")
    }

    private val goodList: MutableSet<String?> = HashSet<String?>()
    ...
    fun generateSimpleTags(modelName: String): ArrayList<String?> {
