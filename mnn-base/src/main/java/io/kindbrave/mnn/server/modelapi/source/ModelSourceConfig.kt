...
package io.kindbrave.mnn.server.modelapi.source
...
class ModelSourceConfig {
    ...
    fun getRepoConfig(modelId: String): RepoConfig? {
        ...
        if (repoConfig == null) {
            ...
            if (splits.size == 2) {
                val modelScopeId = "KindBrave/" + splits[1]
                repoConfig = RepoConfig(modelScopeId, modelId, modelId)
            } else {
                val modelScopeId = "KindBrave/$modelId"
                val huggingFaceId = "KindBrave/$modelId"
